-- 20260831_notification_shown_once_per_user.sql
-- Ensure notifications are shown once per user per notification, regardless of multiple or future orders

-- 1. Deduplicate existing acknowledgments in service_notification_acknowledgments keeping the earliest acknowledgment
DELETE FROM public.service_notification_acknowledgments a
WHERE a.id NOT IN (
    SELECT DISTINCT ON (user_id, notification_id) id
    FROM public.service_notification_acknowledgments
    ORDER BY user_id, notification_id, acknowledged_at ASC
);

-- 2. Drop old composite unique constraint (user_id, order_id, notification_id)
ALTER TABLE public.service_notification_acknowledgments 
DROP CONSTRAINT IF EXISTS service_notification_acknowle_user_id_order_id_notification_key;

-- 3. Add new unique constraint (user_id, notification_id)
ALTER TABLE public.service_notification_acknowledgments 
DROP CONSTRAINT IF EXISTS service_notification_acknowledgments_user_notif_unique;

ALTER TABLE public.service_notification_acknowledgments 
ADD CONSTRAINT service_notification_acknowledgments_user_notif_unique 
UNIQUE (user_id, notification_id);

-- 4. Update get_pending_service_notifications RPC
