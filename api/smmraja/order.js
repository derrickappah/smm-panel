...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (!isAdmin) return res.status(403).json({ error: 'Unauthorized: Direct provider access restricted to admins' });

        const { service, link, quantity, runs, interval, comments } = req.body;

        if (!service) return res.status(400).json({ error: 'Missing required field: service', field: 'service' });
        if (!link) return res.status(400).json({ error: 'Missing required field: link', field: 'link' });
        if (quantity === undefined || quantity === null) return res.status(400).json({ error: 'Missing required field: quantity', field: 'quantity' });
