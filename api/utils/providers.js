...
async function fetchTikstaRecentOrders(limit) {
    const { url: TIKSTA_API_URL, key: TIKSTA_API_KEY } = await getProviderCredentials('tiksta', 'https://tiksta.com/api/v2');

    if (!TIKSTA_API_KEY) return [];
    ...
    try {
        ...
        console.error('Tiksta fetch orders failed:', e);
        return [];
    }
}

async function fetchTikstaStatus(providerOrderId) {
    const { url: TIKSTA_API_URL, key: TIKSTA_API_KEY } = await getProviderCredentials('tiksta', 'https://tiksta.com/api/v2');

    if (!TIKSTA_API_KEY) throw new Error('Tiksta API key not configured');
    ...
    if (!response.ok) throw new Error(`Tiksta API error: ${response.status}`);
    return await response.json();
}

async function placeTikstaOrder(service, link, quantity, comments) {
    const { url: TIKSTA_API_URL, key: TIKSTA_API_KEY } = await getProviderCredentials('tiksta', 'https://tiksta.com/api/v2');
    ...
    if (!response.ok) {
        ...
        const errorMessage = errorData.error || errorData.message || `Tiksta API error: ${response.status}`;
        console.error(`[PROVIDER FAILURE] tiksta: ${errorMessage}`, { status: response.status, details: errorData });
...
async function fetchSmmTakeRecentOrders(limit) {
    try {
        const { url: SMMTAKE_API_URL, key: SMMTAKE_API_KEY } = await getProviderCredentials('smmtake', 'https://smmtake.com/api/v2');

        if (!SMMTAKE_API_KEY) {
            console.warn('SMM Take API key not configured, skipping order fetch');
            return [];
        }
        ...
        console.error('SMM Take fetch orders failed:', e);
        return [];
    }
}

async function fetchSmmTakeStatus(providerOrderId) {
    const { url: SMMTAKE_API_URL, key: SMMTAKE_API_KEY } = await getProviderCredentials('smmtake', 'https://smmtake.com/api/v2');

    if (!SMMTAKE_API_KEY) throw new Error('SMM Take API key not configured');
    ...
    if (!response.ok) throw new Error(`SMM Take API error: ${response.status}`);
    return await response.json();
}

async function placeSmmTakeOrder(service, link, quantity, comments) {
    const { url: SMMTAKE_API_URL, key: SMMTAKE_API_KEY } = await getProviderCredentials('smmtake', 'https://smmtake.com/api/v2');
