...
export default async function handler(req, res) {
    ...
    try {
        ...
        const cacheKey = 'smm:provider:smmtake:balance';
        const cachedBalance = await getCached(cacheKey);
        if (cachedBalance) return res.status(200).json(cachedBalance);

        const SMMTAKE_API_URL = await getConfig('SMMTAKE_API_URL', 'https://smmtake.com/api/v2');
        const SMMTAKE_API_KEY = await getConfig('SMMTAKE_API_KEY');

        if (!SMMTAKE_API_KEY || SMMTAKE_API_KEY.includes('PLACEHOLDER')) {
            return res.status(500).json({ error: 'SMM Take API key not configured', configIssue: true });
        }
        ...
        try {
            const response = await fetch(SMMTAKE_API_URL, {
                ...
                body: new URLSearchParams({ key: SMMTAKE_API_KEY, action: 'balance' }).toString(),
        ...
        return res.status(500).json({ error: 'Failed to retrieve balance from SMM Take.' });
    }
}
