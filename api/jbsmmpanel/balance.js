...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      // Log response status
      console.log('JB SMM Panel Balance API Response:', {
      ...
      if (!response.ok) {
        ...
        console.error('JB SMM Panel Balance API Error:', {
          status: response.status,
          errorData
        });
      ...
      try {
        ...
        console.error('JB SMM Panel Balance Response Parse Error:', parseError);
        return res.status(500).json({ 
      ...
      console.log('JB SMM Panel Balance API Full Response:', JSON.stringify(data, null, 2));

      // Check for errors in response body (API might return errors with 200 status)
      if (data && typeof data === 'object' && data.error) {
        console.error('JB SMM Panel Balance API returned error in response:', data.error);
      ...
      if (fetchError.name === 'AbortError') {
        console.error('JB SMM Panel balance request timeout after', REQUEST_TIMEOUT, 'ms');
    ...
    const duration = Date.now() - startTime;
    console.error('JB SMM Panel balance error:', {
      error: error.message,
    ...
    return res.status(500).json({ 
      error: 'Failed to retrieve balance from provider. Please try again later.'
    });
  }
}
