 ...
 * @returns {string|null} Mapped status or null if unknown
 */
const mapOldSMMStatus = (oldsmmStatus) => {
...
/**
 * Map SMM Raja status to our status format
 * @param {string} smmrajaStatus - Status from SMM Raja API
 * @returns {string|null} Mapped status or null if unknown
 */
const mapSMMRajaStatus = (smmrajaStatus) => {
  if (smmrajaStatus === null || smmrajaStatus === undefined) return null;

  const statusString = String(smmrajaStatus).trim();
  if (!statusString) return null;
...
const checkSingleOrderStatus = async (order, onStatusUpdate = null) => {
  ...
  try {
    ...
    if (hasSmmRajaId) {
      ...
      statusData = await getSmmRajaStatus(order.smmraja_order_id);
      ...
      // Get status from OldSMM
      const { getOldSMMStatus } = await import('./oldsmm');
      statusData = await getOldSMMStatus(order.oldsmm_order_id);
