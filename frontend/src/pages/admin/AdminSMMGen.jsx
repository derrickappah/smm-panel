...
import { fetchSMMGenServices, getSMMGenBalance } from '@/lib/smmgen';
...
const AdminSMMGen = () => {
  ...
  React.useEffect(() => {
    const savedApiUrl = localStorage.getItem('smmgen_api_url');
    const savedApiKey = localStorage.getItem('smmgen_api_key');
  ...
  const testConnection = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('/api/smmgen/balance', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setConnectionMessage('Successfully connected to SMMGen API');
        ...
        if (errorData.configIssue || errorMessage.includes('not configured')) {
          toast.error('SMMGen API is not configured. Please set SMMGEN_API_KEY and SMMGEN_API_URL in Vercel environment variables.');
        } else {
  ...
  // Fetch available services from SMMGen
  const syncServices = useCallback(async () => {
    ...
    try {
      const services = await fetchSMMGenServices();
      
      if (Array.isArray(services) && services.length > 0) {
        // Log first service structure for debugging
        console.log('Sample SMMGen service structure:', JSON.stringify(services[0], null, 2));
        addLog(`Sample service fields: ${Object.keys(services[0]).join(', ')}`);
        
        setAvailableServices(services);
        addLog(`Successfully fetched ${services.length} services from SMMGen`);
        toast.success(`Fetched ${services.length} services from SMMGen`);
      } else {
  ...
  const importServices = useCallback(async () => {
    ...
    try {
      ...
      for (const service of servicesToImport) {
        try {
          // Extract service ID - SMMGen uses "service" or "id" field (string)
          const smmgenServiceId = service.smmgen_service_id || service.service || service.id || service.service_id || service.serviceId || service.ID;
          ...
          // Extract platform - SMMGen services already have platform mapped
          const rawPlatform = service.platform || service.category || '';
          const normalizedPlatform = normalizePlatform(rawPlatform);

          // Extract service type
          const serviceType = service.service_type || service.type || 'other';

          // Parse rate - SMMGen returns it as a number or string
          const rateValue = service.rate ? parseFloat(service.rate) : 0;

          // Map service from SMMGen format to our database format
          const serviceData = {
            smmgen_service_id: String(smmgenServiceId), // SMMGen IDs are strings
            platform: normalizedPlatform,
            service_type: serviceType,
            name: service.name || 'Unknown Service',
            rate: rateValue, // Use actual rate from API (admin can adjust later)
            min_quantity: parseInt(service.min_quantity || service.min || 0, 10),
            max_quantity: parseInt(service.max_quantity || service.max || 0, 10),
            description: service.description || service.desc || service.category || '',
            enabled: true
          };

          // Validate required fields
          if (!serviceData.name || serviceData.name === 'Unknown Service') {
            addLog(`Service with ID ${smmgenServiceId} has no name, skipping...`);
            errorCount++;
            continue;
          }
          ...
          if (smmgenServiceId) {
            const { data: existing } = await supabase
              .from('services')
              .select('id')
              .eq('smmgen_service_id', String(smmgenServiceId))
              .maybeSingle();
          ...
          addLog(`Error importing "${service.name || 'Unknown'}": ${errorMsg}`);
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-4 sm:p-6 shadow-sm">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">SMMGen API Configuration</h2>
        <p className="text-sm text-gray-600 mb-6">
          Configure your SMMGen API credentials and manage service synchronization
        </p>
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Configure your SMMGen API URL and API Key
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
              <p className="text-sm text-blue-800">
                ...
                <br />
                • <code className="bg-blue-100 px-1 rounded">SMMGEN_API_URL</code> - Base URL for SMMGen API
                <br />
                • <code className="bg-blue-100 px-1 rounded">SMMGEN_API_KEY</code> - Your SMMGen API key
                <br />
                <br />
            ...
            <div>
              ...
              <Input
                id="api-url"
                type="text"
                placeholder="https://api.smmgen.com"
                value={apiUrl}
                onChange={(e) => setApiUrl(e.target.value)}
                className="w-full"
                disabled
              />
              <p className="text-xs text-gray-500 mt-1">Set SMMGEN_API_URL in Vercel environment variables</p>
            </div>
            <div>
              ...
              <Input
                id="api-key"
                type="password"
                placeholder="Enter your SMMGen API key"
                value={apiKey}
                onChange={(e) => setApiKey(e.target.value)}
                className="w-full"
                disabled
              />
              <p className="text-xs text-gray-500 mt-1">Set SMMGEN_API_KEY in Vercel environment variables</p>
            </div>
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Check your SMMGen account balance
            </CardDescription>
          </CardHeader>
            <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Fetch and import services from SMMGen
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Button 
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600">
                            ID: {service.smmgen_service_id || service.id || 'N/A'} | 
                            Platform: {service.platform || service.category || 'Unknown'} | 
                            Min: {service.min_quantity || service.min || 0} - 
                            Max: {service.max_quantity || service.max || 0}
                          </p>
                        </div>
                        {selectedServices.has(index) && (
