...
const fetchServices = async () => {
  ...
  if (userRole === 'seller') {
    primaryQuery = primaryQuery.eq('seller_only', true);
...
const fetchAllPendingOrders = async () => {
  ...
  const orders = (data || []).filter(order => {
    ...
    const hasSmmTakeId = order.smmtake_order_id &&
      String(order.smmtake_order_id).toLowerCase() !== "order not placed at smmtake";

    const shouldInclude = hasSmmgenId || hasSmmcostId || hasJbsmmpanelId || hasWorldofsmmId || hasG1618Id || hasOldSmmId || hasApiOwnerId || hasTikstaId || hasSmmRajaId || hasSmmTakeId;
