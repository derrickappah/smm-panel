...
const PremiumNotificationPopup = ({ user }) => {
  ...
  return (
    <>
          ...
          <CardContent className="px-5 py-4 space-y-3 max-h-[75vh] overflow-y-auto scrollbar-hide">
            ...
            {currentNotification.show_video && currentNotification.video_url && (
              <button 
                ...
                className="w-full flex items-center justify-between p-3 bg-white border border-indigo-100 rounded-xl hover:bg-indigo-50 transition-colors group cursor-pointer"
              >
      ...
      {/* Robust Fullscreen Video Modal */}
      {showVideoModal && currentNotification.video_url && (
        <VideoModal
          videoUrl={currentNotification.video_url}
          title={currentNotification.title || "Video Guide"}
