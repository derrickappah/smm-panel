-- Improve place_order function to better link transactions and prevent duplicates
-- This ensures order transactions are properly linked to balance_audit_log
-- and removes any duplicate manual_adjustment transactions
...
BEGIN
    ...
    -- Find the balance_audit_log entry created by the balance update trigger
    ...
    -- Link transaction to balance_audit_log
    IF v_audit_log_id IS NOT NULL THEN
        UPDATE balance_audit_log
        SET transaction_id = v_transaction_id
        WHERE id = v_audit_log_id;
    ELSE
        -- If no audit log entry found, try to link to any matching entry
        -- Use subquery to get the ID first, then update
        SELECT id INTO v_audit_log_id
        FROM balance_audit_log
        WHERE user_id = p_user_id
          AND transaction_id IS NULL
